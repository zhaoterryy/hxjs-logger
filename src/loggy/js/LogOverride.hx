...
class LogOverride {
    ...
    private static function init() {
        untyped window.console.warn = function() {
            ...
            LogStorage.push({ level:'warn', msg:msgs.toString(), stack:stack });
        ...
        untyped window.console.error = function() {
            var msgs:Array<Dynamic> = Array.from(untyped __js__('arguments'));
            var stack = new Error().stack;
            LogStorage.push({ level:'error', msg:msgs.toString(), stack:stack });
        ...
        untyped window.console.log = function() {
            var msgs:Array<Dynamic> = Array.from(untyped __js__('arguments'));
            LogStorage.push({ level:'log', msg:msgs.toString() });
